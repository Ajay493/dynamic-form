        ...
        <div *ngIf="!field.value" class="drop-container dropzone" dropZone (hovered)="toggleHover($event)"
          ...
          <p class="m-0">
            ...
            <label class="upload-button">
              <input type="file" multiple="" [id]="field.name" [name]="field.name" [formControlName]="field.name" (change)="field.onUpload($event.target.files); field.value = ($event.target.files[0].name); "> browse
            </label>
            to upload.
          </p>
        </div> 
